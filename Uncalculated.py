...
class Uncalculated:
    ...
    def UncalculatedLoadData(self):
        (node1_x,node1_y,node1_support_x,node1_support_y,node1_fx,node1_fy)=np.loadtxt('temp.txt',skiprows=0,delimiter=' ',dtype=float,usecols=(0,1,2,3,4,5),unpack=True)
        (node2_x,node2_y,node2_support_x,node2_support_y,node2_fx,node2_fy)=np.loadtxt('temp.txt',skiprows=0,delimiter=' ',dtype=float,usecols=(6,7,8,9,10,11),unpack=True)
